...
- **QuickShop-Hikari**: kompiliertes Jar mit CFR dekompiliert (die Source-API ist
  ...
  Ursprünglich über `ShopManager#getAllShops()` gelesen - das liefert aber nur
  Shops, deren Chunk gerade geladen ist (`ContainerShop#isValid()`/`isLoaded()`
  intern), auf einer nicht erkundeten Welt praktisch nichts. Stattdessen jetzt
  `DatabaseHelper#listShops()` (`com.ghostchu.quickshop.api.database.DatabaseHelper`,
  liefert `ShopRecord` mit `DataRecord`/`InfoRecord` - ein reiner SQL-Join über
  QuickShops eigene Tabellen, kein Chunk-Zugriff nötig) und für die Item-Daten
  `QuickShop.getInstance().platform().decodeStack(String)` (die öffentliche API
  hat dafür keine Entsprechung - `Util#deserialize()` ist die ältere,
  YAML-basierte Variante und scheitert an modernen, komponentenbasierten
  Item-Daten, empirisch verifiziert). `Shop#getPrice()`/`getOwner()`/
  `shopType().isBuying()`, `ShopSuccessPurchaseEvent` weiterhin für Preis/
  Besitzer/Typ am lebenden Objekt bzw. für die Transaktionshistorie.
...
## Test mit echten Towny-Produktionsdaten

Zusätzlich zum synthetischen Testserver wurde ein echter Towny-Datenexport eines
Produktionsservers importiert (146 Städte, 9.204 Residents, 8 Nationen, 51.362
Townblocks, 723,9 MB). Dabei aufgefallen, aber **kein Bug in diesem Plugin**:

- **Kaputte Dateinamen-Kodierung beim Entpacken unter Windows**: Der Export wurde als
  `.tar.gz` mit UTF-8-Dateinamen (z.B. `Allianz_des_weißen_Adlers.txt`) geliefert.
  Windows' eingebautes `tar.exe` (bsdtar/libarchive) dekodiert nicht-ASCII-Dateinamen
  beim Entpacken falsch (offenbar CP437 statt UTF-8, auch mit `chcp 65001` reproduzierbar)
  - aus "ü" wurde "├╝" als tatsächlicher Dateiname auf der Platte, nicht nur eine
    Anzeige-Macke. Towny erkannte diese kaputten Namen beim Laden dann korrekt als
    "invalid name" und hat die betroffenen Städte/Nationen automatisch zu "Town1",
    "Nation1" usw. umbenannt (Datenverlust der echten Namen). Gefixt durch Entpacken
    mit Git-for-Windows' GNU-`tar` (`LC_ALL=C.UTF-8 tar -xzf ...`) statt dem
    Windows-eigenen `tar.exe` - liefert korrekte UTF-8-Dateinamen. Kein Bug im Plugin,
    aber relevant für jeden, der auf Windows größere UTF-8-Datenexporte entpackt.
- **`TownyCollector` skaliert unauffällig**: 146 Städte × insgesamt 51.362 Townblocks
  synchron durchlaufen (für Plot-Zählung/neue-Plots-Erkennung) lief ohne spürbare
  Verzögerung im Server-Tick - der bestehende Code brauchte keine Anpassung.
- **Stadtkassen zeigen 0,00** für die importierten Städte: Towny speichert Kontostände
  nicht in den eigenen Flatfiles, sondern fragt sie über Vault beim Economy-Provider
  ab (`Town#getAccount()`). Der importierte Datensatz bringt aber nur Townys eigene
  Daten mit, nicht die zugehörigen Bankkonten des Produktions-Economy-Providers - auf
  dem Testserver (TheNewEconomy, eigene kleine Testdaten) existieren diese Konten
  schlicht nicht. Gleiches Muster wie bei der bereits dokumentierten
  `Economy.mv.db`-Untersuchung: keine "versteckten" Daten, die das Plugin übersieht,
  sondern zwei unabhängige Datenquellen (Towny-Flatfiles vs. Economy-Backend), von
  denen nur eine importiert wurde.

## Echter Disk-Full-Vorfall während der Entwicklung (nützlicher Zufallstest)

Während des obigen Towny-Imports ist die Entwicklungsmaschine tatsächlich randvoll
gelaufen (u.a. durch Townys eigene, nicht rotierte automatische Backups - nichts
mit diesem Plugin zu tun). Das hat einen echten Bug in diesem Plugin aufgedeckt,
den synthetische Tests nie gefunden hätten:

- **`QuickShopDatabase#replaceShops` und `RegionMarketDatabase#replaceRegions`
  blieben nach einem fehlgeschlagenen Schreibvorgang dauerhaft kaputt**: Beide
  Methoden riefen `connection.setAutoCommit(false)` ... `commit()` ...
  `setAutoCommit(true)` aber ohne try/catch/finally auf. Schlug ein Schreibvorgang
  fehl (hier: `SQLITE_FULL`, weil die Festplatte voll war), wurde die Exception
  einfach weitergereicht - `setAutoCommit(true)` wurde nie erreicht. SQLite hatte
  die Transaktion intern längst selbst verworfen, aber die JDBC-Connection dachte
  noch, eine sei offen. Ergebnis: jeder folgende Aufruf schlug mit
  `SQLITE_ERROR: cannot commit - no transaction is active` fehl - **auch nachdem
  wieder Speicherplatz frei war**. Ohne Neustart des Plugins wäre die
  QuickShop-/AdvancedRegionMarket-Registry dauerhaft nicht mehr aktualisiert worden.
  Gefixt: beide Methoden fangen jetzt SQLExceptions ab, versuchen ein `rollback()`
  (das selbst fehlschlagen darf, wird ignoriert) und setzen `setAutoCommit(true)`
  garantiert in einem `finally`-Block zurück - genau das Muster, das
  `TraderDatabase` über `beginBatch()`/`commitBatch()`/`rollbackBatch()` schon
  hatte, hier aber bei den beiden neueren Registry-Spiegelungen fehlte. Beim
  genaueren Hinsehen hatte `TraderDatabase#rollbackBatch()` selbst noch eine
  kleinere Variante desselben Bugs: `setAutoCommit(true)` stand hinter
  `rollback()` im selben try-Block, wurde also übersprungen, falls `rollback()`
  seinerseits wirft (genau das passiert, wenn SQLite die Transaktion durch den
  Fehler bereits selbst verworfen hat) - jetzt in einem eigenen `finally`.
